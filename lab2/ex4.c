...
void evalEnv(char** tokenArr, int readTokenNum);
        ...
        for (int i = 0; i < readTokenNum; i++) {
            if (strcmp(tokenArr[i], "|") == 0) {
                ...
                if (code == 0) {
                    ...
                    //SET
                    setenv(curTokenArr[1], curTokenArr[2], 1);
                } else if (code == -2) {
                    //UNSET
                    int len = strlen(curTokenArr[1]);
                    for (int i = 1; i < len; i++) {
                        curTokenArr[1][i - 1] = curTokenArr[1][i];
                    }
                    curTokenArr[1][len - 1] = '\0';
                    unsetenv(curTokenArr[1]);
                } else {
                    int pid = fork();
                    if (pid == 0) {
                        removeQuote(curTokenArr, i - tokenIter);
                        evalEnv(curTokenArr, i - tokenIter);
                        removeQuote(curTokenArr, i - tokenIter);
        ...
        if (finalCode == 0) {
            ...
            if (code == 0) {
                ...
                if (pid == 0) {
                    ...
                    evalEnv(curTokenArr, readTokenNum - tokenIter);
                    removeQuote(curTokenArr, readTokenNum - tokenIter);
                    execv(command, curTokenArr);
                } else {
                    waitpid(pid, NULL, 0);
...
void evalEnv(char** tokenArr, int readTokenNum) {
    for (int i = 0; i < readTokenNum; i++) {
        int len = strlen(tokenArr[i]);
        if (len > 0 && tokenArr[i][0] == '$') {
            int subLen = strlen(tokenArr[i]);
            for (int j = 1; j < subLen; j++) {
                tokenArr[i][j - 1] = tokenArr[i][j];
            }
            tokenArr[i][subLen - 1] = '\0';
            char* env = getenv(tokenArr[i]);
            if (env == NULL) {
                strcpy(tokenArr[i], "");
            } else {
                strcpy(tokenArr[i], env);
            }
        }
    }
}
